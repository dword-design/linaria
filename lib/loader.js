...
var workspaceRoot = findYarnWorkspaceRoot();
...
module.exports = function loader(content, inputSourceMap) {
  ...
  var root = workspaceRoot !== null ? workspaceRoot : process.cwd();
  var outputFilename = path.join(path.isAbsolute(cacheDirectory) ? cacheDirectory : path.join(process.cwd(), cacheDirectory), path.relative(root, this.resourcePath.replace(/\.[^.]+$/, '.linaria.css')));
  var resolveOptions = {
